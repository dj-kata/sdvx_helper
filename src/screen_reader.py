...
from src.define import (
    ...
    HASH_ONRESULT2,
    HASH_ONRESULT_HEAD,
    ONDETECT_RGBSUM_THRESHOLD,
    ONRESULT_ENABLE_HEAD,
    RECT_SELECT_JACKET,
    RECT_SELECT_NOV,
    RECT_SELECT_ADV,
    RECT_SELECT_APPEND,
    ...
    RECT_SELECT_LAMP,
    RECT_HAS_EXSCORE,
    RECT_SELECT_SCORE_LARGE,
    RECT_SELECT_SCORE_SMALL,
    RECT_SELECT_EXSCORE,
    RECT_INFO_JACKET,
    RECT_INFO_TITLE,
    RECT_INFO_LV,
...
_DIGIT_68_MIN_DIST = (
    2  # 6/8混同: ベスト距離がこれ未満なら確信度高とみなし輝度チェックをスキップ
)
...
class ScreenReader:
    ...
    def __init__(
        self, song_db: SongDatabase, orientation: screen_orientation | None = None
    ):
    ...
    def _auto_detect_orientation(self, img: Image.Image) -> screen_orientation | None:
        ...
        for orient in screen_orientation:
            ...
            if (
                self._check_onselect(normalized, threshold=8)
    ...
    def _check_onresult(img: Image.Image, threshold: int = 10) -> bool:
        ...
        if (
            abs(HASH_ONRESULT1 - h0) >= threshold
            or abs(HASH_ONRESULT2 - h1) >= threshold
        ):
            return False
    ...
    @staticmethod
    def _read_digit(
        img: Image.Image, rect: tuple, hash_dict: dict, threshold: int = 10
    ) -> str:
    ...
    def _sample_brightness(
        img: Image.Image, rect: tuple, rel_y: float, rel_x: float
    ) -> float:
        """桁領域内の相対座標 (rel_y, rel_x) 周辺 3×3px の平均輝度を返す。"""
        l, t, r, b = rect
        px = l + max(1, int((r - l) * rel_x))
        py = t + max(1, int((b - t) * rel_y))
        return float(np.array(img.crop((px, py, px + 3, py + 3)).convert("L")).mean())
    ...
    @staticmethod
    def _read_digit_multi(
        img: Image.Image, rect: tuple, hash_dicts: list, threshold: int = 10
    ) -> str:
    ...
    @staticmethod
    def _match_hash(img: Image.Image, rect: tuple, candidates: list[tuple]) -> any:
    ...
    def _detect_result_score_display_mode(
    ...
    def read_from_result(self) -> Optional[dict]:
        ...
        try:
            ...
            return {
                ...
                "is_exscore_mode": score_display_mode
                == result_score_display_mode.exscore,
                "detect_mode": (
...
if __name__ == "__main__":
    ...
    def _debug_hash_dist(img, rects, hash_dict, label):
        ...
        for i, rect in enumerate(rects):
            ...
            lower_left = float(
                np.array(img.crop((px, py, px + 3, py + 3)).convert("L")).mean()
            )
            ...
            top_right = float(
                np.array(
                    img.crop((px_tr, py_tr, px_tr + 3, py_tr + 3)).convert("L")
                ).mean()
            )
            print(
                f"  {label}[{i}]: best={best_k}(d={dists[best_k]})"
    ...
    def read_select(f):
        ...
        print(
            f"  lamp RGB=({r_mean:.1f},{g_mean:.1f},{b_mean:.1f})  sat={s:.3f}  val={v:.3f}  hue={h:.3f}"
        )
