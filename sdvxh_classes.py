...
hdl_formatter = logging.Formatter(
    "%(asctime)s %(filename)s:%(lineno)5d %(funcName)s() [%(levelname)s] %(message)s"
)
...
class OnePlayData:
    ...
    def get_vf_single(self, lv):
        ...
        if type(lv) == int:
            ret = int(
                lv * score * coef_grade * coef_lamp * 20 / 10000000
            )  # 42.0とかではなく420のように整数で出力
        self.vf = ret
        return ret
    ...
    def __eq__(self, other):
        ...
        return (
            ...
            and (self.cur_score == other.cur_score)
            and (self.pre_score == other.pre_score)
    ...
    def disp(self):  # debug
        print(
            f"{self.title}({self.difficulty}), cur:{self.cur_score}, pre:{self.pre_score}({self.diff:+}), exscore:{self.pre_exscore}->{self.cur_exscore}, lamp:{self.lamp}, date:{self.date}"
        )
...
class SDVXLogger:
    ...
    def load_settings(self):
        ...
        for k in default_val.keys():
            if not k in ret.keys():
                print(
                    f"{k}が設定ファイル内に存在しません。デフォルト値({default_val[k]}を登録します。)"
                )
                ret[k] = default_val[k]
        self.settings = ret
        ...
        try:
            ...
            for i, d in enumerate(self.alllog):
                if d.cur_score > 10000000:
                    dellist.append(i)
                    print(
                        f"誤検出リザルトを自動削除しました。({d.title}, {d.difficulty}, {d.cur_score})"
                    )
            ...
            with open("out/rival_tmp.csv", encoding="utf-8") as f:
                ...
                for i, r in enumerate(csvr):
                    ...
                    try:
                        ...
                        logger.debug(
                            f"rival data error! (title:{r[0]}, difficulty:{difficulty}, best_score:{best_score}, best_lamp:{best_lamp})"
                        )
            ret[name] = tmp
            vf = 0.0
    ...
    def pop_illegal_logs(
        self, title: str, difficulty: str, score: int, exscore: int, lamp: str
    ):
        ...
        for i, d in enumerate(self.alllog):
            if (d.title == title) and (d.difficulty.lower() == difficulty.lower()):
                if (
                    (d.cur_score > score)
                    or (d.cur_exscore > exscore)
                    or (lamp_table.index(d.lamp) < lamp_table.index(lamp))
                ):
                    target.append(i)
                    print(f"不正データ?: {d.cur_score:,}, {d.lamp}, ({d.date})")
                    print(
                        f"judge: {(d.cur_score > score)}, {d.cur_exscore > exscore}, {lamp_table.index(d.lamp) < lamp_table.index(lamp)}, {d.lamp}, {lamp}"
                    )
                ...
                if type(lv) == int:
                    ...
                    if maya2info is not None:  # maya2のマスタ上に楽曲情報が存在する場合
                        if maya2info["s_tier"] is not None:
                            f.write(
                                f"    <gradeS_tier>{maya2info['s_tier'][5:]}</gradeS_tier>\n"
                            )
                        f.write(f"    <PUC_tier>{maya2info['p_tier']}</PUC_tier>\n")
                    elif min(19, lv) in (17, 18, 19):  # 対象LvならS難易度表を取得
                        tmp = self.gen_summary.musiclist[f"gradeS_lv{min(19, lv)}"].get(
                            title
                        )
                        if tmp != None:
    ...
    def push_today_updates(self):
        ...
        if len(self.alllog) > 0 and self.alllog[-1] not in self.today_updates:
            ...
            if pre_best is None:
                ...
                (pre_best.best_score < tmp.cur_score)
                or (pre_best.best_exscore < tmp.cur_exscore)
                or (lamp_table.index(pre_best.best_lamp) < lamp_table.index(tmp.lamp))
            ):
                push_ok = True
            ...
            if push_ok:
                logger.info(
                    f"today_updates.append: {tmp.title}, {tmp.difficulty}, lamp:{tmp.lamp}, score:{tmp.cur_score}, ex:{tmp.cur_exscore}"
                )
                duplicate = False
                for i, s in enumerate(
                    ...
                    if (s.title == tmp.title) and (s.difficulty == tmp.difficulty):
                        duplicate = True
                        self.today_updates[i].cur_score = max(
                            self.today_updates[i].cur_score, tmp.cur_score
                        )
                        self.today_updates[i].cur_exscore = max(
                            self.today_updates[i].cur_exscore, tmp.cur_exscore
                        )
                        self.today_updates[i].lamp = lamp_table[
                            max(
                                lamp_table.index(self.today_updates[i].lamp),
                                lamp_table.index(tmp.lamp),
                            )
                        ]
                        logger.info(
                            f"merged!, i={i}, title:{s.title}, difficulty:{s.difficulty}, score:{self.today_updates[i].cur_score}, exscore:{self.today_updates[i].cur_exscore}, lamp:{self.today_updates[i].lamp}"
                        )
                if not duplicate:
    ...
    def gen_vf_onselect(self, title: str, difficulty: str):
        ...
        Args:
            ...
            difficulty (str): 譜面難易度
        """
        if (title != self.pre_onselect_title) or (
            difficulty != self.pre_onselect_difficulty
        ):  # 違う曲になったときだけ実行
            logs, info = self.get_fumen_data(title, difficulty)
            lv = info.lv
            dat = []
    ...
    def update_rival_view(self, title: str, difficulty: str):
        ...
        """
        if (
            (title != self.pre_onselect_title)
            or (difficulty != self.pre_onselect_difficulty)
        ) and (len(self.rival_names) > 0):  # 違う曲になったときだけ実行
            infos = []
            ...
            for name in self.rival_names:  # tmp: 1人分
                ...
                for s in tmp:  # 1曲分
                    if (s.title == title) and (
                        s.difficulty.lower() == difficulty.lower()
                    ):
                        s.player_name = name
                        s.is_maya2 = False
                        s.me = False
            ...
            with open("out/rival.xml", "w", encoding="utf-8") as f:
                ...
                for i, (info, r) in enumerate(zip(infos_sorted, rank)):
                    ...
                    f.write(f"        <best_score>{info.best_score}</best_score>\n")
                    f.write(
                        f"        <best_lamp>{info.best_lamp.lower()}</best_lamp>\n"
                    )
    ...
    def get_fumen_data(self, title: str, difficulty: str):
        ...
        for p in reversed(self.alllog):
            if (p.title == title) and (p.difficulty == difficulty):
                ...
                best_lamp = (
                    p.lamp
                    if lamp_table.index(p.lamp) > lamp_table.index(best_lamp)
                    else best_lamp
                )
                best_score = (
                    p.cur_score
                    if (p.cur_score > best_score) and (p.cur_score <= 10000000)
                    else best_score
                )
                best_exscore = (
                    p.cur_exscore if (p.cur_exscore > best_exscore) else best_exscore
                )
    ...
    def update_stats(self):
        ...
        with open(self.filename_stats, "w", encoding="utf-8") as f:
            ...
            f.write(
                f"    <total_vf_diff>{self.total_vf - self.vf_pre:.3f}</total_vf_diff>\n"
            )
        ...
        if self.gen_summary.is_result(img):
            ...
            if ocr != False:
                ...
                if playdat not in self.alllog:
                    ...
                    logger.debug(
                        f"added! -> {playdat.title}({playdat.difficulty}) {playdat.cur_score} {playdat.lamp}"
                    )
            else:
        ...
        for f in self.gen_summary.get_result_files():
            ...
            if self.gen_summary.is_result(img):
                ...
                if ocr != False:
                    ...
                    self.gen_summary.result_parts["jacket_org"].save(
                        f"jackets/{hash}.png"
                    )
    ...
    def gen_best_csv(self, filename):
        try:
            with open(
                filename, "w", encoding="utf-8", errors="ignore", newline=""
            ) as f:
                writer = csv.writer(f)
                writer.writerow(
                    ["title", "difficulty", "Lv", "score", "lamp", "volforce"]
                )
                for i, p in enumerate(self.best_allfumen):
                    diff = p.difficulty.replace("APPEND", "").upper()
                    lamp = (
                        p.best_lamp.replace("exh", "maxxive")
                        .replace("hard", "exc")
                        .replace("clear", "comp")
                        .upper()
                    )
    ...
    def gen_alllog_csv(self, filename):
        try:
            ...
            with open(
                filename, "w", encoding="shift_jis", errors="ignore", newline=""
            ) as f:
                writer = csv.writer(f)
                writer.writerow(
                    ["title", "difficulty", "Lv", "score", "lamp", "volforce", "date"]
                )
                for i, p in enumerate(reversed(self.alllog)):
                    lv = "??"
                    if p.title in self.gen_summary.musiclist["titles"].keys():
                        lv = self.gen_summary.musiclist["titles"][p.title][
                            3 + list_diff.index(p.difficulty)
                        ]
                    vf = p.get_vf_single(lv)
                    diff = p.difficulty.replace("APPEND", "").upper()
                    lamp = (
                        p.lamp.replace("exh", "maxxive")
                        .replace("hard", "exc")
                        .replace("clear", "comp")
                        .upper()
                    )
    ...
    def analyze(self) -> str:
        ...
        # ツイート用文字列作成
        vfdiff = (
            f" (+{self.total_vf - self.vf_pre:.3f})"
            if self.total_vf > self.vf_pre
            else ""
        )
...
class OneUploadedScore:
    ...
    def disp(self):
        print(
            f"rev:{self.revision}, music_id:{self.music_id}, difficulty:{self.difficulty}, score:{self.score:,}, exscore:{self.exscore:,}, lamp:{self.lamp}"
        )
    ...
    def delete(self, revision: int, music_id: str):
        for i, s in enumerate(self.scores):
            if s.music_id == music_id and s.revision == revision:
                self.scores.pop(i)
                logger.info(
                    f"uploaded score deleted! (id:{s.music_id}, rev:{s.revision})"
                )
                return True  # 削除成功
        return False  # 削除失敗
...
class ManageMaya2:
    ...
    def is_alive(self):
        ...
        try:
            if self.params.get("maya2_testing"):
                r = requests.get(
                    self.params.get("maya2_url_testing") + "/", params=payload
                )
            else:
    ...
    def get_musiclist(self):
        ...
        try:
            ...
            if self.params.get("maya2_testing"):
                r = requests.post(
                    self.params.get("maya2_url_testing") + "/api/testing/export/musics",
                    headers=header,
                )
            else:
                r = requests.post(
                    self.params.get("maya2_url_v1") + "/api/v1/export/musics",
                    headers=header,
                )
            js = r.json()
    ...
    def get_rival_scores(self):
        ...
        try:
            ...
            if self.params.get("maya2_testing"):
                r = requests.post(
                    self.params.get("maya2_url_testing")
                    + "/api/testing/export/rival_scores",
                    headers=header,
                )
            else:
                r = requests.post(
                    self.params.get("maya2_url_v1") + "/api/v1/export/rival_scores",
                    headers=header,
                )
            js = r.json()
            dict_lamp = {
            ...
            for m in self.master_db:
                if m.get("title") == title:
                    for c in m.get("charts"):
                        ...
                        if (
                            (fumen == "APPEND")
                            and (c["difficulty"] not in ("NOV", "ADV", "EXH"))
                        ) or (c["difficulty"] == fumen.upper()):
                            ret = c
                            break
        except Exception:
            return None
    ...
    def upload_best(
                ...
                if key not in tmp_maya2.keys():
                    if upload_all:
                        ...
                        tmp_maya2[key] = {
                            "music_id": music.get("music_id"),
                            "difficulty": chart.get("difficulty"),
                            "best_score": song.cur_score,
                            "exscore": song.cur_exscore,
                            "lamp": lamp,
                        }
                else:
                    if upload_all:
                        tmp_maya2[key]["best_score"] = max(
                            tmp_maya2[key]["best_score"], song.best_score
                        )
                        tmp_maya2[key]["exscore"] = max(
                            tmp_maya2[key]["exscore"], song.best_exscore
                        )
                    else:
                        tmp_maya2[key]["best_score"] = max(
                            tmp_maya2[key]["best_score"], song.cur_score
                        )
                        tmp_maya2[key]["exscore"] = max(
                            tmp_maya2[key]["exscore"], song.cur_exscore
                        )

                    lamps = ["PLAYED", "COMP", "EX_COMP", "MAX_COMP", "UC", "PUC"]
                    print(lamp, tmp_maya2[key]["lamp"])
                    tmp_maya2[key]["lamp"] = lamps[
                        max(lamps.index(lamp), lamps.index(tmp_maya2[key]["lamp"]))
                    ]
                    print(
                        f"duplicated data was updated -> key:{key}, data:{tmp_maya2[key]}"
                    )
            else:
                cnt_ng += 1
                print(f"not found in maya2 db!! title:{key}, diff:{song.difficulty}")
                logger.debug(
                    f"not found in maya2 db!! title:{key}, diff:{song.difficulty}"
                )
        ...
        checksum = hmac.new(
            secret, payload.encode(encoding="utf-8"), hashlib.sha256
        ).hexdigest()
